...
const ALLOWED_PARAMS = new Set(['season', 'limit']);
...
export default async function handler(request: Request) {
  ...
  // Extract subpath
  const subpath = pathname.replace(/^\/api\/football-data/, '');
  ...
  const targetUrl = new URL(`https://api.football-data.org/v4${subpath}`);
